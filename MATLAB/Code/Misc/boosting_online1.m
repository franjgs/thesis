...
for i = 1 : N
    ...
    maxEffectiveNum=find(alpha<1e-6,1)-1;
    for m = 1 : maxEffectiveNum%M
        predictions(m) = svmpredict(y_training(i, :), x_training(i, :), learners{m});
    end
    predictions = sign(predictions * alpha);
    if predictions==y_training(i,:)
        predictions = zeros(size(y_testing, 1), M);
        for m = 1 : maxEffectiveNum%M
            predictions(:, m) = svmpredict(y_testing, x_testing, learners{m});
        end
    ...
    x = [sv_instances{1}; x_training(i, :);];
    y = [sv_labels{1}; y_training(i, :);];
