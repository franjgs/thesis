...
Figure~\ref{fig:svm_linear_classify} shows a scenario where an SVM is used to classify two sets of points from partially overlapping Gaussian distributions, generated as listed in Listing~\ref{lst:svm_linear_classify}. The same dataset, when classified using an RBF kernel results in a decision boundary that looks like as shown in Figure~\ref{fig:svm_rbf_classify}
